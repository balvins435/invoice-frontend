...
import { Button, EmptyState, Input, LoadingState, MetricCard, Modal } from '@/components/ui';
          ...
          {!hasBusinesses && !isBusinessLoading ? (
            ...
            <>
              ...
              <section className="rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
                ...
                {showFilters && (
                  <div id="invoice-filters" className="border-b border-slate-100 p-4 dark:border-slate-800">
                ...
                {isLoading ? (
                  <LoadingState title="Loading invoices" description="Fetching the latest records for this business." />
                ) : filteredInvoices.length === 0 ? (
                  <EmptyState title={searchQuery || Object.keys(filters).length ? 'No matching invoices' : 'No invoices yet'} description={searchQuery || Object.keys(filters).length ? 'Try clearing the search or filters.' : 'Create your first invoice to start billing customers.'} action={searchQuery || Object.keys(filters).length ? <Button variant="secondary" onClick={() => { setSearchQuery(''); setFilters({}); }}>Clear filters</Button> : <Link href={ROUTES.createInvoice} className="btn-primary">Create invoice</Link>} />
                ) : (
                  <>
